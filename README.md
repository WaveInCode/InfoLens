# **InfoLens - A Web Scraping Dashboard**
## Description
  Infolens is a Python and CustomTkinter based GUI dashboard, which scrapes data from credible websites such as ScienceDaily, Economic Times, wttr.in and API(s) such as yfinance to provide you personalized info on multi genres - news, finance, and weather. Whether it be the latest science innovations, real time stock data and highlights, or location based weather fetching; InfoLens ensures u stay up to date with the major trends.
 
  
## Tech Stack
 Infolens, as stated, is powered by multiple websites and api(s). In general, we have used:
 1) Python ver. 3+
 2) CustomTkinter and Tkinter for GUI implementation
 3) Requests and BeautifulSoup4 for webscraping
 4) PIL and Pygame for sound and icons.
 5) Other modules such as datetime, random, and webbrowser for added functionality.

Please refer to requirements.txt for more information.

## Features
Infolens has a ton of features, the most impressive ones are listed below:
1. Fullscreen GUI built with Python and CustomTkinter
2. Live news scraping from Google News or Economic Times
3. Dynamic stock market data using yfinance or scraping
5. Modular, object-oriented architecture with clear file separation
6. Manual refresh and dynamic content updates
7. Sound effects for interaction and transitions
8. Hover effects, glow effects, and custom icons
9. Toast-style offline notification with auto-dismiss
10. Graceful error handling for invalid input and network issues
11. Animated title bar with sliding text and blinking cursor
12. Polished design with navy/charcoal gradients and soft shadows
    Also, click **space** for an animation :)
    
## Compatibility
### Infolens Ver. 1.0 has been tested in *windows 10/11*

**Note:** Some features are Windows-specific:
- Fonts: Uses **Arial, Bahnschrift, Fira Code, Calibri** as the primary fonts.
- DPI awareness: Ensures sharp display on high-resolution screens.
- Sounds: Uses **.mp3** files played via **pygame**.

Other platforms (macOS, Linux) may work, but fonts, DPI scaling, or sounds may not render as intended.



## Screenshots
### Home Page
<img width="866" height="846" alt="Screenshot 2025-10-16 084030" src="https://github.com/user-attachments/assets/5a998ece-06d7-47af-bbc5-8874d6d9dd7e" />

### News Dashboard
<img width="900" height="939" alt="Screenshot 2025-07-26 195245" src="https://github.com/user-attachments/assets/f5a0ff91-d05f-4c2b-bc3f-3ec19c345a9b" />

### Finance Dashboard
<img width="888" height="940" alt="Screenshot 2025-07-26 195515" src="https://github.com/user-attachments/assets/5814bc86-96cc-4779-94f4-b77094fb30fe" />

### Weather Dashboard
<img width="1026" height="937" alt="Screenshot 2025-08-03 183746" src="https://github.com/user-attachments/assets/f7eba3af-a288-4c73-95de-bde2b7cc1d8f" />

## Upcoming Features
Even if this is currently a school project, I want to continue adding great additions in the coming versions for user accessibility. Here are a few you can expect:
* For news dashboard, personalized news topics that span further than just science, such as politics, agriculture, computer science or more. Along with this, I would like more news items shown rather than the current limit( being 4)
* An enhanced finance highlights section with more usability, as well as customizable stock info providing data on a majority of stocks rather than the ones that currently exist.
* Improved layout system to support different screen sizes

## Feedback/Suggestions
I would love to hear your invaluable feedback, this will help make the app more suitable and relevant for you!

**Share your thoughts here:** [Suggestions](https://github.com/WaveInCode/InfoLens/issues/1#issue-3520182437)

## Developer Notes
InfoLens originates as a school exhibition project, which is thought of and made completely by me. It took about 2 weeks to complete the first working version, and overall 3 weeks to finalize it. Though, pushing through each obstacle I now feel happy and proud upon my creation, and intend to further bolster its applications. Its made with lots of love and effort, I hope u like it :)
